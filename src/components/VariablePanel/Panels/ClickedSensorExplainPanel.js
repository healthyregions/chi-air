...
export const ClickedSensorExplain = ({ pop }) => {
  ...
  const explanations = {
    aqi: `The Air Quality Index, AQI, is a color-coded, 0 to 500 scale used to communicate how polluted the air is, and the associated health risks. Higher AQI values indicate higher pollution levels and greater health concerns. An AQI under 100 is generally safe, while values over 100 indicate risks, particularly for sensitive groups.`,
    pm25: 'PM2.5 refers to fine particulate matter—tiny solid particles and liquid droplets in the air that have a diameter of 2.5 micrometers or smaller. To put their size in perspective, a single human hair is about 70 micrometers wide, making the largest PM2.5 particle 30 times smaller than the width of a hair.',
    no2: 'Nitrogen dioxide (NO₂) is a highly reactive, reddish-brown, pungent gas that acts as a primary indicator of traffic-related air pollution and a key component of smog, largely emitted from burning fuels in vehicles, power plants, and industrial facilities. As a "criteria" air pollutant, it is tightly regulated due to its role in causing respiratory illness, particularly asthma, and contributing to the formation of particulate matter PM 2.5 and ground-level ozone.'
  }
  ...
  return(
    <Grid size={11}>
      ...
      <Grid container spacing={2} marginTop={'1.5rem'}>
        <Grid size={12}>
          <SGHeader>AQI - Air Quality Index<TTSAudioPlayer text={explanations.aqi} /></SGHeader>
          <SGBody>{explanations.aqi}</SGBody>
        </Grid>
        <Grid size={12}>
          <SGHeader>PM 2.5 - Particulate Matter 2.5<TTSAudioPlayer text={explanations.pm25} /></SGHeader>
          <SGBody>{explanations.pm25}</SGBody>
        </Grid>
        <Grid size={12}>
          <SGHeader>NO₂ - Nitrogen Dioxide<TTSAudioPlayer text={explanations.no2} /></SGHeader>
          <SGBody>{explanations.no2}</SGBody>
        </Grid>
        {/*<Grid size={12}>
