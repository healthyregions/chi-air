...
const baseUrl = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQg4Hwl6JPLnDjI0zA7tvljOUQrRowGitpxz1S2R84rDxf8mAoUJq2eOR9HVl9SrHbeA-KoHz3kIMnm/pub?output=csv'
const baseSheets = {
...
/** Reads/parses a JSON directory into an array of JSON objects */
...
/** Reads/parses a Markdown directory into an array of JSON objects */
const zipMarkdownDirectoryToJsonArray = (mdDir, predicate = (jsonObj) => jsonObj) => {
    return fs.readdirSync(mdDir).filter(f => f.endsWith('.md')).map(filename => {
        ...
        const [ header, frontmatterYaml, content ] = fullContents.split('---\n');
        header && console.warn(`WARNING - header contained unexpected text: ${header}`);
        // DEBUG && console.log('Initial Post Metadata fetched: ', { header, frontmatterYaml, content });
        ...
        DEBUG && console.log('Post JSON parsed / compiled: ', finalJson);
        return finalJson;
    });
};

/**
 * Predicate/helper function that injects a new field into an object.
 * Calls the getValue() function to retrieve the value.
 */
...
/**
 * Predicate function that injects a "tagline" field into each Post JSON object
 * Tagline consists of the first ~25 words of the Markdown content (ignoring headers), and ends with ellipses
 */
